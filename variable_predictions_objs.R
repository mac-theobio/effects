...
qoi_age_pred_plot <- (plot(qoi_age_pred, ci=FALSE)
	...
	+ annotate('text', x = 0.5, y = 4.5, label = "Delta(x)==1",parse = TRUE,size=5, colour="red") 
	+ annotate('text', x = -1, y = 5.5, label = paste0("Delta(y)==", round(coef(qoi_mod1)[[2]],3)),parse = TRUE,size=5, colour="red") 
	+ annotate('text', x = 0, y = 6.5, label = paste0("Marginal~effect(x)==", round(coef(qoi_mod1)[[2]],3)),parse = TRUE,size=5, colour="blue") 
