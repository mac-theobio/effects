...
load("mediate_bin_preds.rda")
load("mediate_preds.rda")
...
For models with random effects components, another of adjustments can be made to correct for bias induced by the random effects terms. In the population approach, we treat the random effects terms as additional non-focal predictors and simply make adjustment to Equation~\ref{eq:pop_eta}. In particular
%
...
\mathrm{\eta}_i &= \beta_0 + \beta_{\mathrm{A}}\mathrm{Age}_i \nonumber\\
\mathrm{Age}_i &\sim \mathrm{Normal}(0, 1) \nonumber\\
\beta_0 &= 5 \nonumber\\
\beta_{\mathrm{A}} &= 0.8 \nonumber\\
i &= 1,\cdots, 500
...
\mathrm{status}_i &\sim \mathrm{Binomial}(1, \mathrm{P(status}_i = 1)) \nonumber\\
\mathrm{logit(P(status}_i = 1)) &= \eta_i \nonumber\\
\mathrm{\eta}_i &= \beta_0 + \alpha_{0,\mathrm{h}[i]} + \beta_{\mathrm{A}}\mathrm{Age}_i \nonumber\\
...
\mathrm{status}_{{\mathrm{h}[i]\mathrm{s}}} &\sim \mathrm{Binomial}(1, P(\mathrm{status}_{{\mathrm{h}[i]\mathrm{s}}} = 1)) \nonumber\\
\mathrm{logit}(P(\mathrm{status}_{{\mathrm{h}[i]\mathrm{s}}} = 1)) &= \beta_{0,s} + \alpha_{0,\mathrm{h}[i]\mathrm{s}} + \beta_{\mathrm{A}}\mathrm{Age}_i + \beta_{\mathrm{W}}\mathrm{Wealthindex}_i + \beta_{\mathrm{Ls}}\mathrm{Latent}_{i\mathrm{s}} \nonumber\\
...
\section{Appendix A: Additional simulations}

\subsection{Mediated variables}

We consider simple indirect mediation example such that $x$ has direct effect on $y$ which in turn has effect on $z$ but $x$ has no direct effect on $y$, i.e., $x \rightarrow y \rightarrow z$. In particular:
%
\begin{align}\label{sim:simple_mediate}
z &= \beta_0 + \beta_{yz} y + \epsilon_z \nonumber\\
y &= \beta_{xy} x + \epsilon_y \nonumber\\
x &\sim \mathrm{Normal(0, 1)} \nonumber\\
\epsilon_y &\sim \mathrm{Normal(0, 1)} \nonumber\\
\epsilon_z &\sim \mathrm{Normal(0, 1)} \nonumber\\
\beta_0 &= 5 \nonumber\\
\beta_{xy} &= 1 \nonumber\\
\beta_{yz} &= 1.5
\end{align}
%
We fit two models -- \emph{non-mediated} which models \emph{z} as a function of \emph{x} and the \emph{mediated} which models \emph{z} as a function of \emph{x} and \emph{y}. In both cases, we compare predictions without bias correction (none) with those corrected for bias (bias corrected). Figure~\ref{fig:pred_mediate_plots} compares the predictor effect of \emph{x} on \emph{z} for the two models. For simple linear model, the predicted averages (model center estimates) are similar to the observed average ($\bar{z}$) and none bias corrected and bias corrected predictions are very similar. 
%
\begin{figure}[H]
\begin{center}
<<pred_mediate_plots, echo=FALSE, fig=TRUE, message=FALSE>>=
print(pred_mediate_plots)
@
\end{center}
\caption{The non-mediated models z as a function of x while the mediated models z as a function of both x and y. The horizontal dotted red, blue and black lines are the observed, none bias corrected and bias corrected prediction averages, respectively. The vertical dotted grey lines are the average of the focal predictor, x. The dotted blue and black dotted lines are predictor effects of x on z.}
\label{fig:pred_mediate_plots}
\end{figure}
%

We extend the model described in \ref{sim:simple_mediate} to generate binary outcome 
%
%
\begin{align}\label{sim:simple_mediate_bin}
\mathrm{zbin} &\sim \mathrm{Binomial(1, P(zbin = 1))} \nonumber\\
\mathrm{logit(P(zbin = 1))} &= z
\end{align}
%
and then fit two generalized linear models together with the comparisons similar to the ones described above. The results are shown in Figure~\ref{fig:pred_mediate_bin_plots}. For non-mediated model, the bias corrected and non-corrected  predictions are very similar and their averages are very similar to the observed average. However, for the mediated model, the two approaches give very different estimates but bias corrected predicted average is relatively close to the observed average.
%
\begin{figure}[H]
\begin{center}
<<pred_mediate_bin_plots, echo=FALSE, fig=TRUE, message=FALSE>>=
print(pred_mediate_bin_plots)
@
\end{center}
\caption{The non-mediated models zbin as a function of x while the mediated models zbin as a function of both x and y. The horizontal dotted red, blue and black lines are the observed, none bias corrected and bias corrected prediction averages, respectively. The vertical dotted grey lines are the average of the focal predictor, x. The dotted blue and black dotted lines are predictor effects of x on zbin.}
\label{fig:pred_mediate_bin_plots}
\end{figure}
%
