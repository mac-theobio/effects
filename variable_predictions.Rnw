...
%
\begin{align*}
\mathrm{hh~size} &= \beta_0 + \beta_{\mathrm{A}}\mathrm{Age}_i + \beta_{\mathrm{M}}\mathbf{I}_{\mathrm{Gender}_i=Male} + \epsilon_i \\
\mathrm{Age}_i &\sim \mathrm{Normal}(0.2, 1) \\
\mathbf{I}_{\mathrm{Gender}_i=Male} &\sim \mathrm{Sample}(\mathrm{Male}, 0.6) \\
\epsilon_i &\sim \mathrm{Normal}(0, 1) \\
\beta_0 &= 1.5 \\
\beta_{\mathrm{A}} &= 0.8 \\
\beta_{\mathrm{M}} &= 0.2 \\
