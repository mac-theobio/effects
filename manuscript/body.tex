...
\end{center}
\caption{{\bf A comparison of min- and center- anchored effects.} The center-anchored effects are based on the mean of the focal predictor while the min-anchored effects are based on the minimum of the focal predictor. As we mentioned earlier, the choice of the anchor do not affect the main estimates (central trend lines). \jd{Can we use zero anchor instead of min?}} 
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{quantity_of_interest.ggp.pdf}
\caption{{\bf A comparison of prediction, effect and marginal effect.} A: The black solid line is the prediction (same as effect), while the dashed lines indicate the changes along the \code{age} and \code{household size} axis. For a linear model with no interaction, the marginal effect is the slope of the prediction line. B: The marginal effect (ME) at representative values of the focal input variable \code{age} together with the $95\%$ confidence intervals. For simple OLS models, the ME is constant across all the values of \code{age} chosen. \jd{The definitions should be shorter and it's definitely weird to have a figure in the definitions. Maybe you should consider making a box? Also, if we're going to have this figure, we should try to illustrate a more difficult example for comparison.}}
