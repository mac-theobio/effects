...
The most common way of choosing the representative values is taking unique levels or quantiles of the focal input variable if discrete or continuous, respectively.  The predictions are then calculated by holding the non-focal input variables at their reference point (values chosen for the non-focal input variables, e.g., means), while varying the focal input variable, with the goal that the estimates represent how the model responds to the changes in the focal input variable \citep{fox2009effect, hanmer2013behind}. This procedure generates -- \emph{predictor effects} \citep{fox2009effect}, \emph{marginal predictions} \citep{leeper2017package} or \emph{estimated marginal means} \citep{lenth2018package}. 
...
\NEW{In this article, we refer to these quantities as the central (outcome) estimates for an ``average case'' in a population; and the associated confidence intervals will either be a prerequisite for prediction or effects plots. In the case of effects plots, we can also choose an anchor (value chosen for the focal input variable). The concept of effects plots has been presented in the literature -- Shi et al. \citep{shi_evidence_2017} used effects plots to compare the difference in sexual risk behaviors between circumcised and uncircumcised men.}
\dya{In this article, we refer to these quantities as predictions and effects since they should, for example, tell us what we would expect the response to be at a particular value or level of the focal input variable, for an ``average case''. As mentioned above, in our case, the key difference between predictions and effects is in the computation of the confidence intervals. The concept of effects has been presented in the literature -- Shi et al. \citep{shi_evidence_2017} used effect-styled plots to compare the difference in sexual risk behaviors between circumcised and uncircumcised men.}
...
\MOVE{One may be interested in the uncertainties associated with the focal input variable only, excluding other uncertainties due to other non-focal input variables -- effects. However, currently, the two packages do not provide a straightforward way do achieve this. To illustrate this, we generate predictions of $x_2$ from Equation~\ref{eq:simple_inter_higher_no_interaction} together with the associated $95\%$ confidence intervals, as shown in Fig~\ref{fig:justify_ci_plots}. 
%
\begin{figure}
\begin{center}
...
\jd{Are we kind of going backward now from the comparison section? Should all of these definitions come first?}
\BC{I agree. We can move all the package comparisons to simulation section. \MOVE{}}

\jd{This \P\ seems unnecessary. Why not just say “To illustrate \ldots", and then jump to the next \P (“, we consider")}
\BC{How is this?}
...
\jd{We need to explain why we think this is a problem, not just state that it is “usually important".}
...
\jd{I don't understand how this \P\ describes two separate problems. What source of bias is being described in the first case? Are you sure it exists? What does Lenth say about the bias they correct for?}
...
\jd{What are the key ideas if any from the rest of this \P?. I would like to clearly describe what happens with the log link, and also what happens near each end in a logit link. Why is this a place to talk about the explicit Taylor?}
