...
To estimate the central estimate of an outcome plot on the response scale, we need a link function $g$ and appropriate values of the focal predictor $X^{f}$ and reference point for the non-focal predictors $\uset{{\bX}}$. As previously mentioned, we can either use mean-based or whole-sample-based reference point. 
...
\yfi  &= \underset{f}{\textrm{mean}} ~ g^{-1} (\hat{\eta}^\star_j),
\end{align}
%
...
%
\begin{align*}
\etahfi(x_{2i}, x_{3j}, {\uset{\bar{x}}_1}) = \beta_0 + \beta_1 \uset{\bar{x}}_1 + \beta_2x_{2i} + \beta_3x_{3j} + \beta_{23}x_{2i}x_{3j}.
\end{align*}
%
...
\jd{We don't need so much emphasis on the technical details (e.g., our package does something with one less step. Not clear if we need anything from this \P.}
The computation of $\bxo - \bafc$ affects only the intercepts and non-focal predictors -- the slopes and variance corresponding to the focal predictors are not affected. This means that we can still generate effects without necessarily centering the predictors prior to model fitting. 
