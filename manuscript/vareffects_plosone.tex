...
\begin{itemize}
\item \textbf{Input variables}: These are measured variables (and may be transformed)
\item \textbf{Predictors}: These are statistical variables i.e., those terms which are entered into the model. They include the main effects and also polynomials of input variables
\item \textbf{Anchor}: The value of the focal variable (input or predictor) used to calculate CIs. Common choice is the average of the focal predictor
\item \textbf{Reference point}: Values chosen for the non-focal predictors, and can include population averages, quantiles or population values
\end{itemize}
