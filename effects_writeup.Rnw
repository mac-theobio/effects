...
First, we include the intercept variance in the variable effects:
...
<<echo=FALSE>>=
class(contr_pred_all) <- c("jdeffects", "data.frame")
...
@
\end{center}
\caption{Variable effects with intercept and other non-focal predictor uncertainties included. The legend compares the model with non contrast (\code{x2u}) to the one with sum to zero contrast (\code{x2c}).}
...
As explained in the previous section, we again eliminate the uncertainties due to the intercept term and the non-focal variables. 
...
<<echo=FALSE>>=
class(contr_vcov_pred_all) <- c("jdeffects", "data.frame")
...
<<echo=FALSE>>=
class(contr_mmat_pred_all) <- c("jdeffects", "data.frame")
