...
\section{Yet to figure out}
\begin{enumerate}
\item Handling complex interaction
\item Zero SEs in the case of variance-covariance approach
\end{enumerate}
...
Notice that the modification in each of the method function call to pass either covariance matrix or function. In particular, in \pkg{varpred}, it is passed as \pkg{vcmat}, \pkg{emmeans} and \pkg{effects} as \pkg{vcov.} and \pkg{margins} as \pkg{vcov}. However, this seems not to work currently in \pkg{cplot} which computes the predictions in \pkg{margins}.
